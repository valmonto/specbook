 ...
 * auto-removed when UNCHANGED — build worktrees always have commits). A 7-task
 * run once left 16 worktrees totalling ~4 GB plus a stray Vite listener. This
 * reaper is the periodic / start-of-run broom.
 *
 ...
 * └───────────────────────────────────────────────────────────────────────────┘
 *
 * WHY cwd, and why this is safe: the live specbook.valmonto.com runs in dev/
 * watch mode ON THIS BOX — api on :3000 (child of `nest start --watch`) and web
 * on `vite :5173`, BOTH with cwd in the MAIN checkout (apps/{api,web}), object
 * storage in docker on :9000. None of those have a cwd under `.claude/worktrees/`,
 * so the discriminator keeps every one of them. The rule is proven in
 ...
 * `--apply` casually on the live box — a mistake here can take the site down.
 *
 * USAGE:
 ...
 * :9000), whose cwds are siblings of the worktrees root, never inside it.
 *
...
function main() {
  ...
  if (apply) {
    ...
    console.log('\nApplied. Verify the live site still answers: curl -s -o /dev/null -w "%{http_code}" https://specbook.valmonto.com/');
  } else {
