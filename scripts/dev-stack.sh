...
#
# Every build subagent boots a disposable stack (NestJS api + Vite web against a
# throwaway `sb_*` Postgres DB) to drive its browser check, then is supposed to
# tear it down. When the subagent's shell dies mid-check (failure, timeout,
# interrupt) the happy-path teardown never runs and the stack is orphaned — a
# 7-task run once left a Vite listener on :5175 and gigabytes of cruft behind.
#
...
#     ports only — never the live :3000/:5173);
#   - the throwaway `sb_*` database (dropdb --force, terminating any lingering
#     connection).
#
# USAGE:
...
#   API_PORT  (default 3002)   throwaway api port  — MUST NOT be 3000 (live api)
#   WEB_PORT  (default 5175)   throwaway vite port — MUST NOT be 5173 (live web)
#   SB_DB     (default sb_devstack_$$)  throwaway DB name — MUST match sb_*
...
#
# SAFETY: this box also runs the LIVE specbook.valmonto.com in dev/watch mode on
# :3000/:5173 with cwd in the main checkout. This script refuses to run on those
# ports, refuses a DB name outside the `sb_` namespace, and only ever kills the
# throwaway ports it was told to use — so it can never take the live site down.
#
set -euo pipefail

API_PORT="${API_PORT:-3002}"
WEB_PORT="${WEB_PORT:-5175}"
SB_DB="${SB_DB:-sb_devstack_$$}"
...
DATABASE_URL="postgresql://${PG_USER}:${PGPASSWORD}@${PG_HOST}:${PG_PORT}/${SB_DB}"
...
if [[ "$SB_DB" != sb_* ]]; then
  echo "dev-stack: refusing DB '$SB_DB' — throwaway DBs must be named sb_* (never valmatic*/specbook*)." >&2
  exit 2
fi
...
cleanup() {
  ...
  dropdb --if-exists --force -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" "$SB_DB" 2>/dev/null || true
...
# --- Boot -----------------------------------------------------------------------
echo "dev-stack: creating throwaway DB $SB_DB and migrating…" >&2
createdb -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" "$SB_DB"
