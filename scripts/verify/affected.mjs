...
// not a directory) — and specbook build agents run in exactly such worktrees.
// So we resolve the graph from `git diff` (git itself works fine in worktrees)
// and pass explicit `--filter <name>` args to pnpm.
//
