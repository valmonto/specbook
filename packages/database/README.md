...
Two details in `tls.ts` are load-bearing rather than stylistic, and the worker's
external-server probe uses the same helper so they exist only once:
...
- **`servername` is never set for an IP host.** Node throws on an IP there, and
  ...
  This crash-looped the worker in production.
- **`checkServerIdentity` is pinned to the host from the URL.** Left alone Node
  ...
  nobody dialed, and fails with a message that blames the certificate.
