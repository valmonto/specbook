...
import { ATTACHMENT_KINDS, ATTACHMENT_STATUSES, ATTACHMENT_SUBJECT_TYPES } from '@pkg/contracts';
 ...
 * polymorphic (no FK) — the app injects a resolver per subject type that
 * answers "does this subject exist in this org".
 *
...
export const attachment = pgTable(
  ...
  (table) => [
    ...
    check(
      'attachment_subject_type_check',
      sql.raw(`subject_type IN (${ATTACHMENT_SUBJECT_TYPES.map((v) => `'${v}'`).join(', ')})`),
    ),
  ],
);
