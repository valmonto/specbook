...
/** Frosted glass surface shared by the menu + submenu popovers. */
function MenuGlass({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      {/* Fairly opaque: a portaled BlurView on Android often can't sample what's
          behind it, so a translucent tint lets content bleed through (looks
          "weird transparent"). Keep it near-solid. */}
      <View
        className="p-1"
  ...
  return (
    <NativeOnlyAnimatedView entering={FadeIn.reduceMotion(ReduceMotion.System)}>
      <DropdownMenuPrimitive.SubContent
        className={cn(
          'border-border overflow-hidden rounded-2xl border shadow-lg shadow-black/20',
          Platform.select({
...
function DropdownMenuContent({
  ...
  // Keep the menu off the screen edges so it never sits glued into a corner.
          ...
          >
            <TextClassContext.Provider value="text-popover-foreground">
              <DropdownMenuPrimitive.Content
                className={cn(
                  'border-border min-w-[12rem] overflow-hidden rounded-2xl border shadow-lg shadow-black/20',
                  Platform.select({
                    web: cn(
