...
import { cycleChain } from './plan-layout';
import {
  V2_CARD_H,
  V2_NODE_W,
  blockersOf,
...
/**
 * Plan v2 — the hand-rolled dependency canvas (no React Flow, no dagre). Ported
 ...
 * goes through the SAME real hooks as v1, so the server keeps enforcing the
...
export interface PlanCanvasV2Props {
  draftTasks: Task[];
  readOnly: boolean;
...
export function PlanCanvasV2({
  ...
  const cardHeight = useCallback((id: string) => heights[id] ?? V2_CARD_H, [heights]);
  ...
  const onDragMove = useCallback((e: globalThis.PointerEvent) => {
    ...
    setPositions((prev) => {
      ...
      const ch = el?.offsetHeight ?? V2_CARD_H;
  ...
  const onLinkMove = useCallback((e: globalThis.PointerEvent) => {
    ...
    if (!fromPos) return;
    const fromH = nodeRefs.current.get(from)?.offsetHeight ?? V2_CARD_H;
            ...
            return (
              <g key={key}>
                ...
                <path
                  d={path}
                  fill="none"
                  markerEnd={`url(#${conflict ? 'plan-v2-arrow-flash' : 'plan-v2-arrow'})`}
                  style={{
