...
describe('AttachmentsService — the three-step protocol', () => {
  ...
  const row = (overrides: Record<string, unknown>) => ({ ...pendingRow, ...overrides });
  ...
  it('rejects an unknown subject type', async () => {
    await expect(
      service.createUpload(human, {
        // Bypasses the Zod layer on purpose to prove the service guards too.
        subjectType: 'invoice' as never,
        subjectId: TASK,
        kind: 'image',
        mimeType: 'image/png',
        sizeBytes: 1000,
  ...
  it('deletes object and row when the upload exceeds the ceiling', async () => {
